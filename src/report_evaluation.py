...
def paired_gru_ridge(rows: pd.DataFrame, bootstrap_samples: int = 2000) -> pd.DataFrame:
    ...
    for horizon, group in paired.groupby("horizon_hours", sort=True):
        ...
        output.append({
            ...
            "gru_track_mean_km": float(group.gru_track_error_km.mean()), "ridge_track_mean_km": float(group.ridge_track_error_km.mean()),
            "track_delta_gru_minus_ridge_km": float(group.track_delta_gru_minus_ridge_km.mean()),
            ...
            "gru_wind_mae_kt": float(group.gru_wind_abs_error_kt.mean()), "ridge_wind_mae_kt": float(group.ridge_wind_abs_error_kt.mean()),
            "wind_mae_delta_gru_minus_ridge_kt": float(group.wind_mae_delta_gru_minus_ridge_kt.mean()),
